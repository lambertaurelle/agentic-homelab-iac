...
1. **Dynamic Node Placement**:
   - Accepts `node_name = var.node_name` (defaulting to `var.app_node_name` / `node-2`, seamlessly overridable to `node-1`).
...
module "my_app_container" {
  ...
  node_name   = var.app_node_name # Defaults to node-2, overridable to node-1
  vm_id       = 701
  hostname    = "my-app"
...
| :--- | :--- | :--- | :--- |
| `node_name` | `string` | `"node-2"` | Proxmox node hosting container (`node-2` or `node-1`). |
