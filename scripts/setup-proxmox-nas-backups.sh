...
#   4. Multi-node storage activation across 'proxmox' and 'tuxmox'
...
NFS_EXPORT="${NFS_EXPORT:-${PLEX_NAS_HOST_PATH:-/mnt/nas/data}}"
...
# Helper function to execute commands on tuxmox node
run_on_tuxmox() {
    local cmd="$1"
    ssh -o StrictHostKeyChecking=no "root@${TUXMOX_HOST}" "${cmd}"
}
...
if ! run_on_master "pvecm status" >/dev/null 2>&1; then
    echo "[-] Error: Unable to reach Proxmox master node at ${PROXMOX_HOST}" >&2
    exit 1
fi
echo "[+] Proxmox master node (${PROXMOX_HOST}) is reachable."

if ! run_on_tuxmox "hostname" >/dev/null 2>&1; then
    echo "[-] Error: Unable to reach Proxmox worker node at ${TUXMOX_HOST}" >&2
    exit 1
fi
echo "[+] Proxmox worker node (${TUXMOX_HOST}) is reachable."
...
# Refresh/check proxmox node
PROXMOX_STATUS=$(run_on_master "pvesm status -storage ${STORAGE_ID}" | awk 'NR>1 {print $3}')
if [ "${PROXMOX_STATUS}" != "active" ]; then
    echo "[*] Reactivating storage on proxmox node..."
    run_on_master "pvesm set ${STORAGE_ID} --disable 0; pvesm status -storage ${STORAGE_ID}"
    PROXMOX_STATUS=$(run_on_master "pvesm status -storage ${STORAGE_ID}" | awk 'NR>1 {print $3}')
fi

# Refresh/check tuxmox node
TUXMOX_STATUS=$(run_on_tuxmox "pvesm status -storage ${STORAGE_ID}" | awk 'NR>1 {print $3}')
if [ "${TUXMOX_STATUS}" != "active" ]; then
    echo "[*] Activating storage mount on tuxmox node..."
    run_on_tuxmox "pvesm set ${STORAGE_ID} --disable 0 >/dev/null 2>&1 || true; pvesm status -storage ${STORAGE_ID} >/dev/null 2>&1 || true"
    sleep 2
    TUXMOX_STATUS=$(run_on_tuxmox "pvesm status -storage ${STORAGE_ID}" | awk 'NR>1 {print $3}')
fi

echo "[+] Node 'proxmox' storage status: ${PROXMOX_STATUS}"
echo "[+] Node 'tuxmox' storage status:  ${TUXMOX_STATUS}"

if [ "${PROXMOX_STATUS}" != "active" ] || [ "${TUXMOX_STATUS}" != "active" ]; then
...
echo "[*] Proxmox Storage Status (proxmox):"
run_on_master "pvesm status -storage ${STORAGE_ID}"

echo "[*] Proxmox Storage Status (tuxmox):"
run_on_tuxmox "pvesm status -storage ${STORAGE_ID}"
