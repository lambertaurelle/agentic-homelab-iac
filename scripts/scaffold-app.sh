...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            ...
            echo "  --type <custom|thirdparty|native>   Workload type"
            echo "  --name <app_name>                   Application name (e.g. genealogy-api)"
            echo "  --node <tuxmox|proxmox>             Target Proxmox node (default: tuxmox)"
            echo "  --repo <github_url_or_slug>         GitHub repository URL or image name"
            echo "  --port <port>                       Application listening port (default: 8080)"
            echo "  --memory <MB>                       RAM allocation in MB (default: 2048)"
            echo "  --cores <cores>                     CPU cores (default: 2)"
            echo "  --disk <GB>                         Disk size in GB (default: 20)"
            echo "  --ctid <ID>                         Proxmox Container ID (default: auto)"
            echo "  --ip <IP_or_dhcp>                   IPv4 address (default: dhcp)"
            echo "  --dry-run                           Print generated files without writing"
...
echo "=============================================================================="
echo "    🏗️  Proxmox Homelab Workload Scaffolder                                   "
echo "=============================================================================="
...
if [ "${INTERACTIVE}" = true ]; then
    if [ -z "${WORKLOAD_TYPE}" ]; then
        echo ""
        echo "Select Workload Architecture:"
        echo "  [1] Custom In-House Application (Docker Compose + Instant CD via Watchtower)"
        echo "  [2] Third-Party Stack (Docker Compose + Nightly Updates)"
        echo "  [3] Native Service (Debian Bare-Metal Package)"
        read -r -p "Enter choice [1-3] (default: 1): " choice_type
        case "${choice_type:-1}" in
    ...
    if [ -z "${APP_NAME}" ]; then
        read -r -p "Application name (e.g. my-app, genealogy): " APP_NAME
    fi
    ...
    read -r -p "Target Proxmox node [tuxmox/proxmox] (default: tuxmox): " node_choice
