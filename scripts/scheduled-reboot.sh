...
# ==============================================================================
# Target Schedule:
#   - Node 1 (proxmox): Sunday 04:00 AM
#   - Node 2 (tuxmox):  Monday 04:00 AM
#
# Pre-Reboot Safety Guards:
#   1. Peer Node Ping Reachability: Verifies peer host is alive.
#   2. Peer AdGuard DNS Resolution:
#      - On proxmox: Validates DNS query against secondary AdGuard.
#      - On tuxmox:  Validates DNS query against primary AdGuard.
...
# Configuration map
PROXMOX_IP="${PROXMOX_IP:-${PROXMOX_NODE_IP:-10.0.0.10}}"
TUXMOX_IP="${TUXMOX_IP:-${TUXMOX_NODE_IP:-10.0.0.20}}"
PRIMARY_ADGUARD_IP="${PRIMARY_ADGUARD_IP:-${ADGUARD_PRIMARY_IP:-10.0.0.201}}"
SECONDARY_ADGUARD_IP="${SECONDARY_ADGUARD_IP:-${ADGUARD_SECONDARY_IP:-10.0.0.202}}"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            ...
            echo "  --node <proxmox|tuxmox>  Explicitly specify host identity"
...
if [ -n "${FORCE_NODE}" ]; then
    ...
    if [[ "$HOST_DETECT" == "proxmox"* ]]; then
        CURRENT_NODE="proxmox"
    elif [[ "$HOST_DETECT" == "tuxmox"* ]]; then
        CURRENT_NODE="tuxmox"
    else
        # Try checking IP address
        if ip addr show 2>/dev/null | grep -q "${PROXMOX_IP}"; then
            CURRENT_NODE="proxmox"
        elif ip addr show 2>/dev/null | grep -q "${TUXMOX_IP}"; then
            CURRENT_NODE="tuxmox"
        else
            log "[-] ERROR: Unable to determine node identity (detected: '${HOST_DETECT}')."
            log "[-] Use '--node proxmox' or '--node tuxmox'."
            exit 1
        fi
    fi
fi
...
if [[ "${CURRENT_NODE}" == "proxmox" ]]; then
    CURRENT_IP="${PROXMOX_IP}"
    PEER_NODE="tuxmox"
    PEER_IP="${TUXMOX_IP}"
    PEER_ADGUARD_IP="${SECONDARY_ADGUARD_IP}"
    SCHEDULED_DAY="Sunday"
elif [[ "${CURRENT_NODE}" == "tuxmox" ]]; then
    CURRENT_IP="${TUXMOX_IP}"
    PEER_NODE="proxmox"
    PEER_IP="${PROXMOX_IP}"
