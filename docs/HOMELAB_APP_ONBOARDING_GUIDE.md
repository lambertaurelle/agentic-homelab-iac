...
flowchart TD
    subgraph CustomApp ["1. Custom In-House Applications (Instant CD)"]
        ...
        WT -->|Instant Container Reload (< 10s)| LiveApp["Target LXC (e.g. CT 701 on node-2)"]
    end
    ...
    subgraph ThirdParty ["2. Third-Party Stacks (Scheduled Updates)"]
        ...
        TP_Cron --> LiveTP["Target LXC (e.g. Vaultwarden, Paperless, Kuma)"]
    end
```
...
Ask the agent:
> *"Scaffold a new custom application named custom-api on node-2 with instant CD for repo youruser/custom-api on port 3000."*
