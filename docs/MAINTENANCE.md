...
gantt
    ...
    Compute Node node-2 Reboot (Mon 04:00)  :crit, 04:00, 15m
...
1. **Stage 1: Proxmox VE Host Updates**
   - Refreshes Proxmox repository indexes (`pve-no-subscription`) and Debian packages, executing `apt-get dist-upgrade -y` with non-interactive dpkg flags on both the Utility Node (`node-1`) and Compute Node (`node-2`).
...
2. **Stage 2: LXC Container OS Updates**
   - Iterates through running containers across both nodes:
     - **Utility Node (`node-1`)**: CT 501 (`adguard-primary`), CT 510 (`cloudflared-primary`), CT 601 (`monitoring`), CT 900 (`mgmt-devops`).
     - **Compute Node (`node-2`)**: CT 502 (`adguard-secondary`), CT 511 (`cloudflared-secondary`), plus all dynamically discovered application containers.
...
3. **Stage 3: Native Application Updates**
   ...
   - **Antigravity CLI & Remote Control Daemon (CT 900)**: Upgrades Antigravity CLI binary via `agy update` and restarts the background user service (`agy-remote-control.service`).
   - **Instance Workloads Hook**: Executes `scripts/instance/update-instance-workloads.sh` if present to update custom native packages.

4. **Stage 4: Docker Compose Stacks (Pull, Update & Prune)**
   - **CT 601 (`monitoring`)**: Updates `/opt/monitoring` (Uptime Kuma).
   - **Custom Application Stacks**: Pulls latest container images and restarts stacks for all active compose directories in `/opt/`.
...
/root/homelab-iac/scripts/update-cluster-stack.sh --node node-1
/root/homelab-iac/scripts/update-cluster-stack.sh --node node-2
...
1. **Guard 1: Peer Node Reachability**:
   - On Utility Node (`node-1`, 10.0.0.10): Pings Compute Node (`node-2`, 10.0.0.20).
   - On Compute Node (`node-2`, 10.0.0.20): Pings Utility Node (`node-1`, 10.0.0.10).
   - **Action if failed**: Aborts reboot immediately with exit code 2.

2. **Guard 2: Peer DNS Service Health**:
   - On Utility Node (`node-1`): Executes `dig @10.0.0.202 google.com` against Secondary AdGuard (CT 502 on `node-2`).
   - On Compute Node (`node-2`): Executes `dig @10.0.0.201 google.com` against Primary AdGuard (CT 501 on `node-1`).
...
- `node-1` (Utility Node): `0 5 * * * update-cluster-stack.sh`, `30 5 * * * scan-security.sh --discord --report`, & `0 4 * * 0 scheduled-reboot.sh`
- `node-2` (Compute Node): `0 5 * * * update-cluster-stack.sh` & `0 4 * * 1 scheduled-reboot.sh`
...
2. Define the container specifications:
   ...
   - Assign target node: `var.utility_node_name` (Utility Node `node-1`) or `var.compute_node_name` (Compute Node `node-2`).
