...
- Creates `stacks/<app-name>/docker-compose.yml` with the application and Watchtower sidecar.
- Creates `stacks/<app-name>/.env.example` with a securely generated `WATCHTOWER_HTTP_API_TOKEN`.
- Creates `stacks/<app-name>/github-deploy-workflow.yml.snippet` for GitHub Actions.

### 3. OpenTofu Validation & Planning
Verify that the generated infrastructure is valid:
```bash
cd tofu
...
- [ ] `stacks/<app-name>/docker-compose.yml` exists with correct container and port bindings.
