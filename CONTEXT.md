...
**Compute Node**:
The secondary Proxmox hypervisor (`tuxmox`) dedicated to compute-heavy application stacks, media processing, and developer workstations.
_Avoid_: Worker node, slave node, compute host

**Utility Node**:
The primary Proxmox cluster lead hypervisor (`proxmox`) hosting core networking, DNS resolvers, ingress tunnels, and management workspaces.
